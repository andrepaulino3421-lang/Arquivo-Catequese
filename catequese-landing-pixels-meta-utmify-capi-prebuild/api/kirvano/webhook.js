import { sendMetaEvent, getPurchaseValue, findValueByKey } from '../_meta.js';
...
function findPhone(input) {
  const direct = findByKey(input, ['phone', 'telephone', 'whatsapp', 'customer_phone', 'buyer_phone', 'client_phone']);
...
function getFallbackPurchaseValue(access) {
  // Fallback usado apenas se a Kirvano não enviar valor total no payload.
  // Ajuste depois se o preço dos order bumps mudar.
  if (access.complete_access) return 27.90;
  if (access.basic_access) return 10.90;
  return undefined;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (access.is_approved && (access.basic_access || access.complete_access || access.activities365_access || access.uno_da_fe_access || access.lifetime_access)) {
      const phone = findPhone(payload);
      const purchaseValue = getPurchaseValue(payload, getFallbackPurchaseValue(access));
      metaPurchase = await sendMetaEvent({
        req,
        eventName: 'Purchase',
        eventId: `kirvano_purchase_${row.order_id}`,
        eventSourceUrl: 'https://dinamicascateque.vercel.app/obrigado',
        userData: {
          email: row.email,
          phone,
        ...
        customData: {
          currency: 'BRL',
          value: purchaseValue,
          content_name: access.complete_access ? 'Pacote Completo Dinâmicas para Catequese' : 'Pacote Básico Dinâmicas para Catequese',
    ...
    return res.status(200).json({ ok: true, saved: true, access, metaPurchase, email: row.email, code: row.code, order_id: row.order_id, data: saved });
  } catch (error) {
